...
<script type = "text/javascript">
    ...
    function uploadComplete(sender,args) {
        ...
        $('#ctl06_txtName').val(args.get_fileName());
        $('#ctl06_txtCamera').val(myPhoto.CameraName);
        $('#ctl06_txtCameraModel').val( myPhoto.CameraModel);
        $('#ctl06_txtExposureBlas').val( myPhoto.ExposureBlas);
        $('#ctl06_txtExposureTime').val( myPhoto.ExposureTime);
        $('#ctl06_txtFlashEnergy').val( myPhoto.FlashEnergy);
        $('#ctl06_txtFlashMode').val(myPhoto.FlashMode);
        $('#ctl06_txtFocalLengh').val(myPhoto.FocalLengh);
        $('#ctl06_txtFstop').val(myPhoto.FStop);
        $('#ctl06_txtMateringMode').val(myPhoto.MateringMode);
        $('#ctl06_txtMaxAperture').val( myPhoto.MaxAperture);
        $('#ctl06_txtISO').val(myPhoto.ISO);
        $('#ctl06_txtSubjectDistance').val(myPhoto.SubjectDistance);
