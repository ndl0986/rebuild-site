...
<%  If bolType = True Then%><div id="section_collection" class="clearfix">
...
<div class="news_cat_content">
    <%  If listNewsInCat.Count > 0 Then%><ul class="list_news clearfix">
        ...
        <% Next%></ul>
    <% If listPaging.Count > 0 Then%><ul class="paging">
        <% For Each pg As Paging In listPaging%><li><a href="<%= pg.PageUrl %>" <% If pg.PageNum = pageNum Then %>class="active"
            <% End If %>>
            <%= pg.PageText %></a></li><% Next%></ul>
    <% End If%><% End If%></div>
<%  Else%><div class="news_cat_content single">
    ...
    <% If listPaging.Count > 0 Then%><ul class="paging">
        <% For Each pg As Paging In listPaging%><li><a href="<%= pg.PageUrl %>" <% If pg.PageNum = pageNum Then %>class="active"
            ...
            <%= pg.PageText %></a></li><% Next%></ul>
    <% End If%><% End If%></div>
<div id="menuTech" runat="server" clientidmode="Static" class="box_gradient">
</div>
