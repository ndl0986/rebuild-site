...
<div class="shop_body">
    <div class="main_tab_alpha">
        ...
        <%For Each shop In tblShop%>
            <li class="sonycenter_info" data-province="<%=shop.Province%>" data-provinces="<%=shop.ProvinceName%>"><div class="table_cell"><%= shop.Name%></div><div class="table_cell"><%= shop.Address%></div><div class="table_cell"><%= shop.OpenTime%> : <%=shop.CloseTime %></div><div class="table_cell"><%= shop.Phone%></div><div class="table_cell"><a href="/Default.aspx?tpl=map&long=<%= shop.Longitude%>&lat=<%= shop.Latitude%>&title=<%= shop.Address%>" class="ico-map" target="_blank">Bản đồ</a></div></li>
        <%Next%>
        </ul>
        <%End If%>
	</div>
